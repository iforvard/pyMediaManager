...
def params(key):
    plugin_params = {
        'auth_required': False,
        # This name must match the name of the plugin file. uTorrent.py => 'name': 'uTorrent'
        'name': 'uTorrent',
    }
...
def dw_torrent(torrents, magnet_urls, host, login, password):
    apiclient = UTorrentAPI(f'http://{host}/gui', login, password)
    for torrent in torrents:
        apiclient.add_file(torrent)
    for url in magnet_urls:
        apiclient.add_url(url)
